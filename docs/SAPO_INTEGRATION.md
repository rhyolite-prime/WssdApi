...
| Dialed service runs the fallback menu instead of its flow | No registry row matched the tried key spellings (or the row's `executable` is empty/invalid), so resolution degraded to file/default — the `[ussd] no workflow ... (tried keys: ...)` line names every spelling attempted: compare against `wssd_registry.ussd_code`. Also look for `[ussd] registry entry ... has no executable blueprint` (row matched, empty flow) or `[ussd] registry lookup failed` (DB error) |
